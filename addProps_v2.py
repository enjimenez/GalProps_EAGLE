...
class GalCat:
    ...
    def add_sigma(self, itype, NumP=100, SF=True, All=False, NSF=False, neutral=False, SF_p=False, feedback=False, nofeed=False):
        if itype == 0:
            if All:
                ...
                if feedback:
                    sigmas, NumPart = self.compute_sigma(itype, NumP=NumP, SF=False, All=True, feedback=True)
                    try: fs.create_dataset('Kinematics/Gas/VelDispFeed_%iP' %NumP, data = sigmas)  
                    except: fs['Kinematics/Gas/VelDispFeed_%iP' %NumP][...] = sigmas
                    
                    try: fs.create_dataset('Kinematics/Gas/NumPart_Feed', data = NumPart)  
                    except: fs['Kinematics/Gas/NumPart_Feed'][...] = NumPart
                    
                elif nofeed:
                    sigmas, NumPart = self.compute_sigma(itype, NumP=NumP, SF=False, All=True, nofeed=True)
                    try: fs.create_dataset('Kinematics/Gas/VelDispnoFeed_%iP' %NumP, data = sigmas)  
                    except: fs['Kinematics/Gas/VelDispnoFeed_%iP' %NumP][...] = sigmas
                    
                    try: fs.create_dataset('Kinematics/Gas/NumPart_noFeed', data = NumPart)  
                    except: fs['Kinematics/Gas/NumPart_noFeed'][...] = NumPart
                    
                    
                else:
                    sigmas, NumPart = self.compute_sigma(itype, NumP=NumP, SF=False, All=True)
                    try: fs.create_dataset('Kinematics/Gas/VelDisp_%iP'%NumP, data = sigmas)  
                    except: fs['Kinematics/Gas/VelDisp_%iP'%NumP][...] = sigmas
                    
                    try: fs.create_dataset('Kinematics/Gas/NumPart', data = NumPart)  
                    except: fs['Kinematics/Gas/NumPart'][...] = NumPart
                
            if SF_p:
                fs = h5.File(Base + 'Galaxies/0%i_%s_%iMpc_%i_%sgalaxies.hdf5' 
                             %(self.snap, self.fend, self.Lbox, self.NP, self.pp), 'a')
                
                sigmas = self.compute_sigma(itype, SF=False, SF_p=True)
                try: fs.create_dataset('Kinematics/Gas/NSF/VelDisp_p', data = sigmas)  
                except: fs['Kinematics/Gas/NSF/VelDisp_p'][...] = sigmas
    ...
    def compute_sigma(self, itype, NumP=100, SF=True, All=False, NSF=False, neutral=False, SF_p=False, feedback=False, nofeed=False):
        ...
        NumPart      = np.zeros((LenGal,30), dtype=int)        
        ...
        if itype == 0:
            ...
            if feedback or nofeed: 
                Tmax         = fh['PartData/MaximumTemperature'].value/10**7.0
                Tvir         = fh['PartData/HostHalo_TVir_Mass'].value/10**7.0
                amax          = fh['PartData/AExpMaximumTemperature'].value
            if SF_p: OnEoS = fh['OnEquationOfState'].value
        ...
        for i in range(LenGal):
            if itype == 0:
                ...
                if feedback:
                    ...
                    #mask = mask & (Tmax[idx][masksgn] > 1.0) & (amax[idx][masksgn] > acut)
                    LenPart = len(np.where(mask)[0])
                
                if nofeed:
                    mask = mask & (Tmax[idx][masksgn] < 1.0)
                    LenPart = len(np.where(mask)[0])
    ...
    def add_ToomreParam(self):
        ...
        VelDispGas  = fs['Kinematics/Gas/VelDisp'].value # [km/s]
        ...
        try: fs.create_dataset('Kinematics/QgasAll',  data = Qgas)
        except: fs['Kinematics/QgasAll'][...] = Qgas
        
        try: fs.create_dataset('Kinematics/Qstar',  data = Qstar)
        except: fs['Kinematics/Qstar'][...] = Qstar
        
        try: fs.create_dataset('Kinematics/QnetAll',  data = Qnet)
        except: fs['Kinematics/QnetAll'][...] = Qnet
